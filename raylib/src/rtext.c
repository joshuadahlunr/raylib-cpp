...
RLFont LoadFontEx(const char *fileName, int fontSize, int *fontChars, int glyphCount)
{
    RLFont font = { 0 };
...
RLFont LoadFontFromMemory(const char *fileType, const unsigned char *fileData, int dataSize, int fontSize, int *fontChars, int glyphCount)
{
    RLFont font = { 0 };
...
bool ExportFontAsCode(RLFont font, const char *fileName)
{
    ...
    if (image.format != PIXELFORMAT_UNCOMPRESSED_GRAY_ALPHA) TRACELOG(LOG_WARNING, "Font export as code: RLFont image format is not GRAY+ALPHA!");
...
void DrawTextCodepoints(RLFont font, const int *codepoints, int count, Vector2 position, float fontSize, float spacing, Color tint)
{
    int textOffsetY = 0;            // Offset between lines (on linebreak '\n')
    float textOffsetX = 0.0f;       // Offset X to next character to draw
